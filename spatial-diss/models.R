# This script runs 4 different Bayesian models using brms
...
set.seed(5511)
mod.2 <- brm(n  ~ 1 + pct.under.2.00 + race.q + pm.q + offset(log(child.pop)),
             data = full.data,
             family = negbinomial(),
             cores = 4, 
             iter = 3000,
...
set.seed(5511)
mod.3 <- brm(n  ~ 1 + pct.under.2.00 + race.q + Fr + pm.q + offset(log(child.pop)),
             data = full.data,
             family = negbinomial(),
             cores = 4, 
             iter = 3000,
...
set.seed(5511)
mod.4 <- brm(n  ~ 1 + pct.under.2.00 + race.q + Fr + pm.q +
                pm.q:Fr + 
